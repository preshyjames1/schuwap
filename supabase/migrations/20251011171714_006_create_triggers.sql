/*
  # Triggers and Automation Migration

  1. Triggers Created
    - `updated_at` timestamp triggers for all tables
    - `update_invoice_balance` - Auto-updates invoice balance when payment is made
    - `calculate_student_fee_final_amount` - Auto-calculates final amount with discount

  2. Functions
    - `update_updated_at_column()` - Updates updated_at timestamp
    - `update_invoice_balance()` - Updates invoice payment status
    - `calculate_student_fee_final_amount()` - Calculates fee with discount
*/
